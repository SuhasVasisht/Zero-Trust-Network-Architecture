...
json.dump(data,open("mine.json","w"))
print(data)
...
data["utsav"] = "NO"
if platform == "linux" or platform == "linux2" or platform=="darwin": json.dump(data,open("linux-final-bad-data.json","w"))

headers = {"Content-type":"application/json","Accept":"text/plain"}

#sending postures to ACE Server:
resp = requests.post("http://{}:9001/submit/".format(IP),data=open("linux-final-data.json","rb"),headers=headers)
...
if (resp.text =="YES"):
    while(True):
        ...
        sleep(5)
        if testCount > 5:
            res = requests.post("http://localhost:9001/scheduled/",data=open("linux-final-bad-data.json","rb"),headers=headers)
            break
        else:
            testCount+=1
            res = requests.post("http://localhost:9001/scheduled/",data=open("linux-final-data.json","rb"),headers=headers)




#redirecting and returning flow to browser:
#



#json.dump(r.text,open("godWillHelpUs.json","w"))
#print(r.text)
